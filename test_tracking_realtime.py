            ...
            for PCs, BBs, list_of_anno in batch:  # tracklet
                ...
                while i < len(PCs):
                    ...
                    if i == 0:
                        ...
                        label_BB = box
                        result_BB = box
                        # print("frame =", i, "bb_index =", bb_index(label_BB), "res_index =", res_index(result_BB))
                        continue
                    else:
                        
                        label_BB, result_BB, frame_to_compare, frame_result = real_time_evaluator.on_data(BBs[i], i)
                        results_BBs.append(result_BB)
                        print("frame_to_compare =", frame_to_compare, "frame_result =", frame_result, "frame =", i)  #, "bb_index =", bb_index(label_BB), "res_index =", res_index(result_BB))
                        ...
                        if real_time_evaluator.can_frame_be_processed():
                            ...
                            (
                                ...
                                new_ref_box,
                                new_this_box,
                            ) = utils.cropAndCenterPC_label_test(
                                this_PC,
                                ref_BB,
                                this_BB,
                                offset=dataset.offset_BB,
                            ...
                            candidate_PCs_torch = candidate_PCs.unsqueeze(
                                0
                            ).cuda()

                            # AGGREGATION: IO vs ONLY0 vs ONLYI vs ALL
                            if (
                                "firstandprevious".upper()
                                in shape_aggregation.upper()
                            ):
                                model_PC = utils.getModel(
                                    [PCs[0], PCs[i - 1]],
                            ...
                            model_PC_torch = utils.regularizePC(
                                model_PC, dataset.input_size, istrain=False
                            ).unsqueeze(0)
                            model_PC_torch = Variable(
                                model_PC_torch, requires_grad=False
                            ).cuda()
                            candidate_PCs_torch = Variable(
                                candidate_PCs_torch, requires_grad=False
                            ).cuda()
                            ...
                            (
                                ...
                                estimation_box,
                                center_xyz,
                            ) = model(model_PC_torch, candidate_PCs_torch)
                            estimation_boxs_cpu = (
                                estimation_box.squeeze(0).detach().cpu().numpy()
                            )
                            ...
                            real_time_evaluator.on_prediction(box, fps_time, i)
                        else:
                            dropped_frames += 1
                    ...
                    this_overlap = estimateOverlap(
                        label_BB, result_BB, dim=IoU_Space
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_root_dir",
        type=str,
        default="./model/car_model/",
        help="output folder",
    )
    parser.add_argument(
        "--data_dir", type=str, default="./data/training", help="dataset path"
    )
    
    parser.add_argument(
        "--data_fps", type=float, default=20.0, help=""
    )
    parser.add_argument(
        "--warmups_needed", type=int, default=10, help=""
    )
    parser.add_argument(
        "--require_predictive_inference", type=str, default=False, help=""
    )
    parser.add_argument(
        "--wait_for_next_frame", type=str, default=False, help=""
    )
    parser.add_argument(
        "--cap_model_fps", type=float, default=None, help=""
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    if args.model != "":
        netR.load_state_dict(
            torch.load(os.path.join(args.save_root_dir, args.model))
        )
    netR.cuda()
    print(netR)
    ...
    for epoch in range(max_epoch):
        Succ, Prec = test(
            ...
            require_predictive_inference=args.require_predictive_inference == "True",
            wait_for_next_frame=args.wait_for_next_frame == "True",
            cap_model_fps=args.cap_model_fps,
            warmups_needed=args.warmups_needed,
            epoch=epoch + 1,
        ...
        logging.info(
            "mean Succ/Prec {}/{}".format(Success_run.avg, Precision_run.avg)
        )
